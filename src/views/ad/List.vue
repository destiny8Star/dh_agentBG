                    ...
                    <div class="main-inner">
                        ...
                        <el-form :inline="true" ref="postData" :model="postData" class="demo-form-inline">
                            <el-form-item label="广告名称">
                                <el-input v-model="postData.ad_name" placeholder="请输入名称" clearable></el-input>
                            </el-form-item>
                            <el-form-item label="广告图名称">
                                <el-input v-model="postData.poster_name" placeholder="请输入名称" clearable></el-input>
                            </el-form-item>
                        ...
                        <template>
                            <el-table ref="multipleTable" :data="tableData" style="width: 100%;min-width: 780px;"
                                ...
                                <el-table-column prop="ad_name" label="广告名称" :key="2">
                                </el-table-column>
                                <el-table-column prop="poster_name" label="广告图名称" :key="3">
                                </el-table-column>
                                <el-table-column prop="start_time" label="投放开始时间" :key="4">
                                </el-table-column>
                                <el-table-column prop="end_time" label="投放结束时间" :key="5">
                                </el-table-column>
                                <el-table-column prop="device_count" label="投放设备数" :key="6" v-if="tabIndex==2">
                                </el-table-column>
                                <el-table-column prop="img_h5_url_pv" label="曝光总量" :key="7" v-if="tabIndex==2">
                                    <template slot-scope="scope">
                                        <p>{{scope.row.img_h5_url_pv ? scope.row.img_h5_url_pv : '--'}}</p>
                                    </template>
                                </el-table-column>
                                <el-table-column prop="status" label="投放状态" :key="8">
                                    <template slot-scope="scope">
                                        <p>{{mapStatus[scope.row.status]}}</p>
                                    </template>
                                </el-table-column>
                                <el-table-column label="操作" min-width='115' :key="9">
                                    <template slot-scope="scope">
                                        <el-button type="text" :class="{colorStyle:scope.row.status!=0}" :disabled="scope.row.status==0"
                                                   @click="offClick(scope.row)">关闭
                                        </el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                //初始化列表
                this.$axios.post('/agent/Advertising/ad/list', this.postData).then(res => {
                    console.log(res);
