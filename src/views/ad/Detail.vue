						...
						<div class="infoBox">
						    ...
						    <el-card class="realBox">
								...
								<div v-for="(ele,i) in infoData" :key="i" class="realBoxItem">
									...
									<div class="realBoxItemVal">
										<p v-if="i!=4">{{ele.value}}</p>
										<div v-else-if="i==4" class="realBoxItemImgBox">
											<div>
                            ...
                            <template>
                                <el-table ref="multipleTable" :data="tableData" style="width: 100%;min-width: 780px">
                                    <el-table-column type="index" fixed width="80" label="编号" :key="1">
	...
	export default {
		...
		methods: {
            ...
            getData() {
				...
				let api = '';
				if( this.type == 1 ){
				    api = '/agent/Advertising/ad/listDeviceByAd'
				}else if( this.type == 2 ){
                    api = '/agent/Advertising/ad/listAdStatistics'
                }
            ...
            getDetail() {
				//初始化列表
                this.$axios.post('/agent/Advertising/ad/get',{"id":this.postData.id}).then(res => {
					console.log('获取详情数据',res);
					this.searchState = false;
                    if (res.data.code === this.$webConfig.httpSuccessStatus) {
                        let info = res.data.data;
                        this.infoData[0].value = this.mapType[info.ad_type];
                        this.infoData[1].value = info.ad_name;
                        this.infoData[2].value = info.valid_time;
                        this.infoData[3].value = info.poster_name;
                        this.infoData[4].value = info.poster_url;
                        this.infoData[4].value2 = info.poster_h5_url;
                        this.infoData[5].value = info.industry_desc;
                        this.infoData[6].value = info.mer_count;
                        this.infoData[7].value = info.device_count;
                        this.infoData[8].value = this.mapStatus[info.status];
            ...
            getEchartsDate() {
				//初始化列表
                this.$axios.post('/agent/Advertising/ad/adStatisticsViewData',{"id":this.postData.id}).then(res => {
					console.log('获取详情数据',res);
					this.searchState = false;
                    if (res.data.code === this.$webConfig.httpSuccessStatus) {
