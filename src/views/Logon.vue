<template>
    ...
    <div class="login1" :style="{height: (getWinHeight) + 'px'}">
        <div class="ly-login-content">
            <h1 class="ly-login-title">{{siteName}}服务商管理后台</h1>
    ...
    export default {
        ...
        methods: {
            ...
            getUserInfo(merAuth) {
                ...
                this.$axios.post("/agent/subject/get/userInfo", {}, {headers}).then(res => {
                    console.log(res);
                    if (res.data.code === this.$webConfig.httpSuccessStatus) {
                        let info = res.data.data;
                        this.$ls.set('accountName', info.real_name);
                        this.$ls.set('accountTel', info.tel);
                        this.$ls.set('agentSubjectId', info.agent_subject_id);
                        this.$ls.set('qrCodeFxPower', info.qr_code_fx_power);
                        if(info.is_perfect==1){
                            this.$router.push('/resellerInfo');
                        }else {
